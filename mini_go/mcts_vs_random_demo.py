...
flags.DEFINE_string("pool_directory", "opponent_pool", "./opponent_pool")
...
def draw_stones(screen, board):
    ...
    GRID_SIZE = 100 # Size of each grid square
    MARGIN = 50     # Margin from screen edge
    STONE_RADIUS = 35  # Radius of each stone
    BLACK = (0, 0, 0)
    WHITE = (255, 255, 255)
...
def main(unused_argv):
    ...
    value_hidden_layers_sizes = [512, 512]
    value_hidden_layers_sizes_kwargs = {
        "replay_buffer_capacity": int(1e5),
        "epsilon_decay_duration": int(0.8 * 10000),
        "epsilon_start": 0.9,
        "epsilon_end": 0.01,
        "learning_rate": 5e-4,
        "learn_every": 64,
    ...
    with tf.Session(graph=rollout_graph) as sess:
        with rollout_graph.as_default():
            ...
            if os.path.exists(FLAGS.pool_directory):
                logging.info("Rollout Agent Loading Opponent Pool.")
            ...
            rollout_agent.restore("saved_model/10000_dqn_fast")
    ...
    with tf.Session(graph=value_graph) as sess:
        with value_graph.as_default():
            ...
            # 加载已有的对手池（如果存在）
            if os.path.exists(FLAGS.pool_directory):
                logging.info("Value Agent Loading Opponent Pool.")
                opponent_pool.load_pool(FLAGS.pool_directory)
            
            value_agent = DQN(sess, 0, env.state_size, env.action_size, value_hidden_layers_sizes, **value_hidden_layers_sizes_kwargs)
            sess.run(tf.global_variables_initializer())
            value_agent.restore("saved_model_value/6000_dqn_value")

        random_agent = RandomAgent(1)

        mcts_agent = MCTS(policy_net=None, rollout_policy_net=None, player_id=0, env=env, max_depth=10, n_simulations=FLAGS.n_simulations, c_param=FLAGS.c_param)
        ...
        for ep in range(FLAGS.num_train_episodes):
            ...
            # 决定是使用对手池中的对手还是随机对手
            if random.random() < FLAGS.pool_sampling_rate and opponent_pool.pool:
                opponent, version = opponent_pool.sample_opponent()
                agents = [mcts_agent, opponent]
            else:
                agents = [mcts_agent, random_agent]
            ...
            if (ep + 1) % FLAGS.save_every == 0:
                if not os.path.exists("saved_model_mcts"):
                    os.mkdir('saved_model_mcts')
                # 保存模型
                agents[0].save(checkpoint_root='saved_model_mcts', checkpoint_name='{}_mcts'.format(ep+1))
