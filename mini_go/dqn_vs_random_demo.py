...
flags.DEFINE_integer("reservoir_buffer_capacity", int(4e6),
    ...
    with tf.Session() as sess:
        ...
        for ep in range(FLAGS.num_train_episodes):
            ...
            if (ep + 1) % FLAGS.save_every == 0:
                if not os.path.exists("saved_model_value"):
                    os.mkdir('saved_model_value')
                agents[0].save(checkpoint_root='saved_model_value', checkpoint_name='{}_dqn_value'.format(ep+1))
        ...
        agents[0].restore("saved_model_value/10000_dqn_value")
        ret = []
